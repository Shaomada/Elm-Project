...
player c = model
    (Player { handle = c, handleDown = False })
    { radius = 30
    , speed = 70
    , color = Color.blue
    , alpha = 0.9
...
zone thing radius = model 
    (Zone { pattern = equalId thing, done = False })
    { radius = radius
    , speed = 0
    , color = thing.color
    , alpha = 0.25
...
zoneEnemy = zone <| enemy pos
zoneBouncy = zone <| bouncy pos
zoneDead = zone <| dead <| block pos


equalId : Thing -> Thing -> Bool
equalId a b = case (a.id, b.id) of
    (Player _, Player _) -> True
    (Enemy _, Enemy _) -> True
    (Bouncy _, Bouncy _) -> True
...
update msg thing = case (msg, thing.id) of
